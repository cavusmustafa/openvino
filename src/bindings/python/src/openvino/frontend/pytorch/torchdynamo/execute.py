...
def partition_graph(gm: GraphModule, use_python_fusion_cache: bool, model_hash_str : str = None):
    ...
    for node in gm.graph.nodes:
        ...
        if node.op == "call_module" and "fused_" in node.name:
            ...
            gm.add_submodule(
                node.target,
                OpenVINOGraphModule(openvino_submodule, partition_id, use_python_fusion_cache, model_hash_str=model_hash_str),
            )
...
def openvino_execute_partitioned(gm: GraphModule, *args, executor_parameters=None):
    ...
    if signature not in partitioned_modules:
        partitioned_modules[signature] = partition_graph(gm, use_python_fusion_cache=use_python_fusion_cache, model_hash_str=model_hash_str)
