...
struct CustomOpTestKey {
    ...
    size_t hash() const;
    bool operator==(const CustomOpTestKey& rhs) const;
};

size_t CustomOpTestKey::hash() const {
...
bool CustomOpTestKey::operator==(const CustomOpTestKey& rhs) const {
    bool retVal = true;
    if (inp0 != rhs.inp0) {
...
bool CustomOpTest::isSupportedOperation(const std::shared_ptr<const ov::Node>& op, std::string& errorMessage) noexcept {
    //try {
    //    if (!std::dynamic_pointer_cast<const ov::opset1::CustomOpTest>(op)) {
    //        errorMessage = "Only opset1 CustomOpTest operation is supported";
    //        return false;
    //    }
    //} catch (...) {
    //    return false;
...
CustomOpTest::CustomOpTest(const std::shared_ptr<ov::Node>& op, const GraphContext::CPtr context) :
        Node(op, context, PassThroughShapeInferFactory()) {
    std::string errorMessage;
    if (!isSupportedOperation(op, errorMessage)) {
        OPENVINO_THROW_NOT_IMPLEMENTED(errorMessage);
    }
    axis = ov::as_type_ptr<ov::op::v1::CustomOpTest>(op)->get_axis();
    //axis = 0;
}

void CustomOpTest::getSupportedDescriptors() {
    if (descs.size())
        return;
...
bool CustomOpTest::created() const {
    return getType() == Type::CustomOpTest;
}

Node::AttrPtr CustomOpTest::initPrimitiveAttr() {
...
void CustomOpTest::initOptimalPrimitiveDescriptor() {
...
void CustomOpTest::createDescriptor(const std::vector<MemoryDescPtr> &inputDesc,
...
void CustomOpTest::prepareParams() {
    ...
    CustomOpTestKey key = {inpDesc, selected_pd->getImplementationType(), axis, *attr};
    auto engine = getEngine();

    auto builder = [&engine](const CustomOpTestKey& key) -> executorPtr {
...
void CustomOpTest::execute(dnnl::stream strm) {
    if (execPtr) {
...
void CustomOpTest::executeDynamicImpl(dnnl::stream strm) {
    execute(strm);
}
